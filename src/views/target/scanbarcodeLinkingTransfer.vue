      ...
      >
        <div class="container-fluid">
          <div class="card shadow-sm border-0 mb-4">
            ...
            >
              <h4 class="mb-0 fw-bold">📦 Laporan Target Transfer TLSI</h4>
            </div>

            <div class="card-body p-4">
              <!-- Filter -->
              <div class="mb-4">
                <a href="/scan-barcode-detail" class="btn btn-warning mt-3 mb-3">
                  <i class="bi bi-box-arrow-in-left me-2"></i>Kembali
                </a>
                ...
                <form @submit.prevent="getReport">
                  <div class="row g-3">
                    ...
                    <div class="col-md-4">
                      ...
                      <input type="date" v-model="filters.pSysBeginDate" class="form-control" />
                    </div>

                    <div class="col-md-4">
                      <label class="form-label fw-semibold">Sampai Tanggal:</label>
                      <input type="date" v-model="filters.pSysEndDate" class="form-control" />
                    </div>
                    ...
                    <div class="col-md-4">
                      ...
                      <select v-model="filters.pProcessType" class="form-select">
                        ...
                        <option value="67">EXP PACK</option>
                        <option value="77">QC AKHIR</option>
                        <option value="68">WHA2P</option>
                        <option value="70">WHA2</option>
                        <option value="71">WHA2I</option>
                        <option value="76">EXP Warehouse</option>
                        <option value="77">WH-TLS</option>
                      </select>
                    </div>
              ...
              <div class="mt-4">
                <div v-if="loading" class="text-center py-5">
                  ...
                  <div class="mt-3 text-muted fs-5">Memuat data, harap tunggu...</div>
                </div>

                <div v-if="data.length > 0">
                  <div class="table-responsive">
                    <table id="targetTransferTable" class="table table-striped table-hover w-100">
                      ...
                      <tfoot>
                        <tr>
                          <th colspan="9" class="text-end">Total All:</th>
...
const calculateDisplayedTotal = () => {
  ...
  const filteredData = dataTableInstance
    ...
    .data()
    .toArray();
  const total = filteredData.reduce((sum, row) => sum + Number(row.xQty || 0), 0);
  displayedTotalQty.value = total.toLocaleString("id-ID");
};

const initializeDataTable = () => {
  nextTick(() => {
    const tableEl = document.getElementById("targetTransferTable");
    if (!tableEl) return;
...
const getReport = async () => {
  loading.value = true;
  try {
    const res = await axios.get(`${API_BASE_URL}/target/linking-scanbarcode-transfer`, {
