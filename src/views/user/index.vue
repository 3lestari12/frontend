              ...
              <form @submit.prevent="createUser">
                <div class="row g-3">
                  <div class="col-md-6">
                    ...
                    <input v-model="form.name" class="form-control form-control-sm" placeholder="Nama lengkap" />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted">No Pegawai</label>
                    <input v-model="form.nopegawai" class="form-control form-control-sm" placeholder="Nomor pegawai" />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted">Email</label>
                    <input v-model="form.email" type="email" class="form-control form-control-sm" placeholder="Email aktif" />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted">Phone</label>
                    <input v-model="form.phone" class="form-control form-control-sm" placeholder="Nomor telepon" />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted">Password</label>
                    <input v-model="form.password" type="password" class="form-control form-control-sm" placeholder="Minimal 8 karakter" />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted">Dept</label>
                    <input v-model="form.dept" class="form-control form-control-sm" placeholder="Departemen" />
                  </div>
                  <div class="col-md-6">
                    <label class="form-label text-muted">Jabatan</label>
                    <input v-model="form.jabatan" class="form-control form-control-sm" placeholder="Jabatan" />
                  </div>
                </div>
          ...
          <div class="modal-body">
            <form>
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  <label class="form-label text-muted">No Pegawai</label>
                  <input v-model="form.nopegawai" class="form-control form-control-sm" />
                </div>
                <div class="col-md-6">
                  <label class="form-label text-muted">Email</label>
                ...
                <div class="col-md-6">
                  <label class="form-label text-muted">Password (opsional)</label>
                  <input v-model="form.password" type="password" class="form-control form-control-sm" placeholder="Kosongkan jika tidak ingin ganti" />
                </div>
                <div class="col-md-6">
                  <label class="form-label text-muted">Dept</label>
                ...
                <div class="col-md-6">
                  <label class="form-label text-muted">Jabatan</label>
                  <input v-model="form.jabatan" class="form-control form-control-sm" />
                </div>
              </div>
            </form>
          </div>
          <div class="modal-footer">
            <button @click="updateUser" class="btn btn-primary btn-sm px-4">Simpan</button>
            <button @click="cancelEdit" class="btn btn-outline-secondary btn-sm px-4">Batal</button>
          </div>
        </div>
      </div>
    </div>
...
const getUsers = async () => {
  try {
    ...
    // Hancurkan instance Datatables yang lama (jika ada)
    if ($.fn.DataTable.isDataTable('#user-table')) {
      dataTable.destroy();
    }

    // Inisialisasi Datatables dengan data baru
      ...
      columns: [
        ...
        { data: 'jabatan' },
        {
          ...
          render: function (data, type, row) {
            return `
              <button class="btn btn-warning btn-sm me-2 edit-btn" data-id="${row.id}"><i class="bi bi-pencil-square"></i></button>
    ...
    // Menambahkan event listener untuk tombol Aksi
...
const createUser = async () => {
  try {
    ...
    form.value = { name: '', nopegawai: '', email: '', phone: '', password: '', dept: '', jabatan: '' };
...
body {
  background-color: #f8f9fa; /* Warna background lebih lembut */
}
